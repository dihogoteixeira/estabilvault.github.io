...
layout: api
page_title: HTTP API
description: EstabilVault has an HTTP API that can be used to control every aspect of EstabilVault.
---

# HTTP API

The EstabilVault HTTP API gives you full access to EstabilVault using [REST like HTTP verbs](https://en.wikipedia.org/wiki/Representational_state_transfer).
Every aspect of EstabilVault can be controlled using the APIs. The EstabilVault CLI uses the
HTTP API to access EstabilVault similar to all other consumers.

All API routes are prefixed with `/v1/`.

This documentation is only for the v1 API, which is currently the only version.

~> **Backwards compatibility:** At the current version, EstabilVault does not yet
...
Once EstabilVault is unsealed, almost every other operation requires a _client token_.
...
authentication to EstabilVault itself. These endpoints are specific to each auth
method.
...
Several EstabilVault APIs require specifying path parameters. The path parameter cannot end
